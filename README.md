...
Keys are only ever read server-side (in `app/api/*/route.ts`) — they're never
sent to the browser.
...
1. Add an entry to `lib/tools.ts` (including a real `createdAt` date — the
   "New" badge is computed from it, not set manually)
2. Add name/description strings to `lib/i18n/en.ts` and `lib/i18n/id.ts`
3. Create `app/(tools)/[slug]/page.tsx` — wrap with `<ToolShell>`, and build
