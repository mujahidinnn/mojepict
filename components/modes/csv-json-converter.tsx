...
/** `direction`, `onSwap` and `initialFile` are supplied by the Universal Converter hub. */
export default function CsvJsonConverterPage({
  ...
  const [pickedMode, setMode] = useState<Mode>("csv-to-json");
  const mode = direction ?? pickedMode;
  const [input, setInput] = useState("");
  const [output, setOutput] = useState("");

  useEffect(() => {
    initialFile?.text().then(setInput);
  }, [initialFile]);
  ...
  const swap = () => {
    if (onSwap) onSwap();
    else setMode((m) => (m === "csv-to-json" ? "json-to-csv" : "csv-to-json"));
    setInput(output);
    setOutput("");
  };
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-4">
        <Tabs value={mode} onValueChange={(v) => setMode(v as Mode)} className={direction ? "hidden" : undefined}>
