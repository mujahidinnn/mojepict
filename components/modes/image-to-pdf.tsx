...
export default function ImageToPdfPage({ initialFile }: { initialFile?: File } = {}) {
  const { t } = useI18n();
  ...
  useEffect(() => {
    // The ref guards against StrictMode's double-run adding the file twice.
    if (initialFile && consumed.current !== initialFile) {
      consumed.current = initialFile;
